...
# class DQN(nn.Module):
#      def __init__(self, inputs,w, outputs):
#          super(DQN, self).__init__()
...
#              return (size - (kernel_size - 1) - 1) // stride + 1
#
#          convw = conv2d_size_out(conv2d_size_out(conv2d_size_out(conv2d_size_out(96))))
#          convh = conv2d_size_out(conv2d_size_out(conv2d_size_out(conv2d_size_out(96))))
...
#     # resized_image_array=gray_array.transpose((2, 0, 1))
#
#     # screen = env.render(mode='rgb_array').transpose((2, 0, 1))
...
#     # screen = torch.from_numpy(screen)
#     # Resize, and add a batch dimension (BCHW)
#     finalimg=torch.tensor(gray_array).to(device).unsqueeze(0).float() / 255.0
#     return finalimg.unsqueeze(0)
#
...
if GRAYSCALE == 0:
    ...
    plt.imshow(get_screen().cpu().squeeze(0).numpy().squeeze(), cmap='gray')
...
# policy_net.load_state_dict(torch.load('save_model/policy_net_best1.pt'))
...
def select_action(state, stop_training):
    ...
    if sample > eps_threshold or stop_training:
        with torch.no_grad():
            ...
            thisaction = policy_net(state).max(1)[1].view(1, 1)
            thisrealaction = action_space[thisaction[0][0].item()]
            # thisfinalaction = torch.tensor([thisrealaction], device=device, dtype=torch.float)
            thisfinalaction = torch.tensor([thisaction], device=device, dtype=torch.long)

            return thisfinalaction
    else:
        randomnumber = random.randrange(n_actions)
        # thisaction = torch.tensor([action_space[randomnumber]], device=device, dtype=torch.float)
        thisaction = torch.tensor([randomnumber], device=device, dtype=torch.long)
...
for i_episode in range(N_EPISODES):
    ...
    for t in count():
        ...
        state_variables, _, done, _ = env.step(action_space[action.item()])
        ...
        if done:
            ...
            print(i_episode)
            thismean = print_scores(t + 1)

            # if (t+1>best_sinlge):
            #     best_sinlge=t+1
            #     torch.save(policy_net.state_dict(), 'savep1/singlebest.pt')  # we save the trained policy model
            # if thismean > bset_mean:
            #     best_mean = thismean
            #     torch.save(policy_net.state_dict(), 'savep1/meanbest.pt')  # we save the trained policy model
    ...
    if i_episode % 10 == 0:
        torch.save(policy_net.state_dict(), 'savepc1/' + str(i_episode) + 'policy.pth')
    #     torch.save(policy_net.state_dict(), 'savep2/singlebest.pt')  # we save the trained policy model

        # policy_net.state_dict()ve
